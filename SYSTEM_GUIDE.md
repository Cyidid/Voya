...
**城市外处理**：返回 400 错误，提示用户切换至「实时规划」，不静默回退至其他城市。
...
集成学习分类器，从用户输入的 16 维特征中预测最适合的旅行类型（8选1），再根据类型生成匹配的行程内容。核心价值是**从偏好模式中学习**，而非写死规则。
...
_build_itinerary()  — 按推荐类型生成行程
    ├── CITY_TYPE_ACTIVITIES 有数据 → 使用精选景点（5城市）
    ├── RULES 景点库有数据   → 复用规则引擎真实景点（25城市）
    └── 无数据 → 通用活动模板（按类型8套）
    ↓
...
| 维度 | 特征 | 说明 |
|------|------|------|
| 行程参数（6维） | days, budget_level, num_people, group_type, has_special, travel_mode | 基本出行参数 |
| 兴趣偏好（7维） | culture, nature, food, shopping, history, nightlife, outdoor | 7类兴趣标志位 |
| 城市语义（3维） | is_domestic, region, city_cost_tier | 是否国内 / 大洲编码 / 城市消费水平 |

城市维度放弃了早期的 7 城 one-hot 方案（每加一个城市就要新增一维、重新训练，且对未见城市完全失效），改用 3 个通用语义维度，覆盖全部 25 个目的地，并可泛化到新城市。
...
**训练数据**：
- 10,000 条程序生成样本，覆盖全部 25 个目的地城市（`_expert_label()` 专家规则函数自动标注）
- **15% 均匀随机标签噪声**（训练集和测试集均施加，模拟真实标注不确定性）
- 80/20 训练/测试分割（random_state=42）
- 测试集准确率：**84.5%**（25 城泛化，噪声数据下的真实场景估计）
...
| 优先级 | 数据源 | 城市范围 |
|--------|--------|----------|
| 1 | `CITY_TYPE_ACTIVITIES`（按推荐类型精选） | 5 城：东京、巴黎、首尔、新加坡、迪拜 |
| 2 | 规则引擎 `RULES` 景点库（复用） | 25 城，按推荐类型映射对应景点分类 |
| 3 | 通用活动模板 | 其余城市兜底 |
...
| `language` | string | 可选 | `zh`（默认）/ `en`，控制行程输出语言 |
...
公共字段（所有系统均返回）：`itinerary`、`agent_type`、`processing_time`、`token_estimate`、`metadata`  
...
> 规则系统特有：`transport_tip_en`、`city_tips_en`（双语贴士）  
> 监督学习特有：`is_uncertain`（置信度是否偏低）
...
本节从学术视角分析系统的负责任 AI 设计，覆盖五大支柱落地和公平性量化分析。公平性指标（AOD）是模型训练时的评估结果，属于算法审计范畴，不在用户界面展示——向终端用户展示固定的训练集统计数字无实际意义，原因见 §9.2。
...
| **城市覆盖** | 固定25城，不支持城市返回错误并提示切换 | 25城训练；城市语义特征可泛化至新城市 | 全球无限制 |
...
**预测路径**：
```
用户偏好特征（16维） → VotingClassifier → 推荐类型（0–7） → 行程生成
```
...
| `group_type` | 家庭出行（3） | 非家庭（单人/情侣/朋友） | +0.163 | 存在偏差 | 家庭出行获得非经济类推荐的概率系统性偏低 |
...
| 系统 | 实现方式 |
|------|---------|
| 规则系统 | 每条推荐可追溯规则库条目；不支持的城市返回 400 错误，不静默回退 |
...
- **规则系统**：不支持的城市返回 400 错误并提示切换至实时规划，不静默回退
...
- 监督学习：`model_accuracy` 字段附带说明，注明"基于合成数据，不代表真实用户行为预测能力"
